...
初回起動時は、必要な設定ファイルが見つからない場合に空のひな形ファイルを自動作成します。
...
> 起動時に、利用可能な場合はアプリの更新確認（最新版の取得）を行います。更新できない場合でも、そのまま起動します。
...
- 大学病院の当直回数を月3コマ以下に制限（当直1コマ、日当直2コマ換算）
...
| 勤務回数上限 (max-assignments.csv) を編集      | 勤務回数上限エディタ | `data/max-assignments.csv`  |
...
### 勤務回数上限エディタ（max-assignments.csv）

病院ごとにセクション分割されたGUIで、担当勤務者の勤務回数上限を設定します。

- 病院セクションのヘッダーをクリックして展開・折りたたみができます
  - 全員が「無制限」（デフォルト）の病院は折りたたまれた状態で表示されます
  - 設定済みの病院は展開状態・太字で表示されます
- 各勤務者に対して **[無制限][勤務なし][1回][2回]** の4択ボタンで上限を設定します
  - 「無制限」: CSVに値を書き込まない（既存の最適化ロジックのデフォルト動作）
  - 「勤務なし」: 上限 0（その病院への割り当てを禁止）
- 勤務者名の横に担当曜日が `(月,金)` のように表示されます
- 「Reload Workers」ボタンで `workers.toml` の変更を再読み込みできます
- 「Save」で保存
...
GUIで使用する主なファイルは次のとおりです。
不足している場合、初回起動時に自動作成されます。
