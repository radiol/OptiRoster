...
2. **負荷分散(重み付き)**: 休日当直を2コマ換算した当直回数の均等化
3. **負荷分散(重みなし)**: 当直の生カウントを均等化
4. **曜日別分散**: 曜日全体での均等分散
5. **当直翌日の外勤回避**: 当直翌日の Day/AM 割り当てを回避
6. **連続休日勤務の回避**: 休日の連続シフトを回避
7. **遠隔地連続勤務の回避**: 遠隔地の連続割り当てを回避
8. **大学病院の当直上限**: 大学病院の当直を月3コマ以下に制限(当直1コマ、日当直2コマ換算)
